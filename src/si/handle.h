...
class HandleStorage : private NonCopyable, private HandleObserver {
 ...
 private:
  ...
  std::deque<HandleEntry> entries_;
  std::deque<HandleId> remove_ids_;
  MutexPtr remove_mutex_;

 public:
  HandleStorage() : remove_mutex_(SDL_CreateMutex()) {}
  ...
  void apply_remove(HandleRemoveObserver<T>* observer = nullptr) {
    ScopedLock lock(remove_mutex_.get());
  ...
  template <typename F>
  void each(F f) const {
    std::for_each(entries_.begin(), entries_.end(), [f](auto& e) {
      if (e.x) {
        f(e.x.get());
      }
    });
 ...
 private:
  ...
  virtual void on_drop(const HandleId& id) override {
    ...
    if (!SDL_AtomicDecRef(&entry.ref_count)) return;
